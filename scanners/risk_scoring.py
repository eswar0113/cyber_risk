...
def getScore(target, save_to_db=True):
    """Run Nmap + VT scans, compute a risk report, and optionally persist to DB."""
    _, nmap_report = scan_and_return_xml(target)
    vt_data        = run_vt_scan(target)
    report         = calculate_risk(nmap_report, vt_data)
    report_dict    = report.__dict__   # converting the report object into a dictionary

    if save_to_db:
        try:
            from db.db import save_scan
            save_scan(report_dict, vt_data)
        except Exception as e:
            print(f"[DB] Could not save scan to database: {e}")
